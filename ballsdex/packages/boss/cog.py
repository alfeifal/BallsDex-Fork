...
class Boss(commands.GroupCog):
    ...
    async def start(
        ...
        await interaction.channel.send((f"# <@&1308512664124788837> The boss battle has begun! {self.bot.get_emoji(ball.emoji_id)}\n-# HP: {self.bossHP} Credits: nobodyboy (Card Art)"),file=file,)
    ...
    async def select(
        ...
        """
        Select the countryball to use against the boss.
        ...
        elif "🃏" in messageforuser or "💎" in messageforuser or "🎃" in messageforuser or "☀️" in messageforuser or "🎴" in messageforuser or "" in messageforuser or "🎄" in messageforuser:
    ...
    async def ongoing(self, interaction: discord.Interaction):
        """
        Check your current damage against the boss.
        """
        await interaction.response.defer(ephemeral=True, thinking=True)
    ...
    async def conclude(self, interaction: discord.Interaction, winner: str):
        ...
        if winner != "None":
            ...
            await interaction.channel.send(f"# Boss has concluded {self.bot.get_emoji(self.bossball.emoji_id)}\nThe boss has been defeated, the winner will be announced shortly.")
        with open("totalstats.txt", "w") as file:
            file.write(f"{total}{total2}")
        with open("totalstats.txt", "rb") as file:
            await interaction.channel.send(file=discord.File(file, "totalstats.txt"))
    ...
    async def join(self, interaction: discord.Interaction):
        ...
        if self.round != 0 and interaction.user.id not in self.users:
            return await interaction.followup.send(
                "It is too late to join the boss, or you have died, ask the hoster to hackjoin you, but it will cost a 1% wipe.", ephemeral=True
            )
        if interaction.user.id in self.users:
            return await interaction.followup.send(
